...
class Interrogator():
    ...
    def chain(
        ...
        phrases = set(phrases)
        if not best_prompt:
            best_prompt = self.rank_top(image_features, [f for f in phrases], revsrse=reverse)
    ...
    def interrogate_simple(self, image: Image, caption: Optional[str]=None) -> str:
        caption = caption or self.generate_caption(image)
        return caption
    ...
    def interrogate(self, image: Image, min_flavors: int=8, max_flavors: int=32, caption: Optional[str]=None) -> str:
        ...
        merged = _merge_tables([self.flavors, self.mediums], self) #[self.artists, self.flavors, self.mediums, self.movements, self.trendings]
        ...
        simple_prompt = self.interrogate_simple(image, max_flavors, caption=caption)
        classic_prompt = self.interrogate_classic(image, max_flavors, caption=caption)
        candidates = [caption, classic_prompt, fast_prompt, best_prompt,simple_prompt]
